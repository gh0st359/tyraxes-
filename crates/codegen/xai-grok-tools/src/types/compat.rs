...
impl CompatConfig {
    ...
    /// Subdirectories scanned for `*.md` rules files. `.tyraxes/rules` and
    /// `.grok/rules` are always included; `.claude/rules` and `.cursor/rules`
    /// are gated on their respective `rules` cell.
    ///
    /// Replaces the hard-coded `RULES_DIRS` constant.
    pub fn rules_dirs(&self) -> Vec<&'static str> {
        let mut dirs = vec![".tyraxes/rules", ".grok/rules"];
...
mod tests {
    ...
    fn skill_config_dirs_all_on_matches_legacy_constant() {
        // Preferred Tyraxes dir first, then legacy `.grok` + vendor dirs.
        assert_eq!(
            CompatConfig::default().skill_config_dirs(),
            vec![".tyraxes", ".grok", ".agents", ".claude", ".cursor"]
        );
    }
    ...
    fn skill_config_dirs_gates_each_vendor() {
        ...
        assert_eq!(
            c.skill_config_dirs(),
            vec![".tyraxes", ".grok", ".agents", ".claude"]
        );

        c.claude.skills = false;
        assert_eq!(c.skill_config_dirs(), vec![".tyraxes", ".grok", ".agents"]);
        ...
        assert_eq!(
            ...
            vec![".tyraxes", ".grok", ".agents", ".cursor"]
        );
    }
    ...
    fn rules_dirs_gates_each_vendor() {
        ...
        assert_eq!(
            c.rules_dirs(),
            vec![".tyraxes/rules", ".grok/rules", ".claude/rules"]
        );
        c.claude.rules = false;
        assert_eq!(c.rules_dirs(), vec![".tyraxes/rules", ".grok/rules"]);
    }
